...
if __name__ == '__main__':
    ...
    if not hparams.no_save_test:
        imgs_name = glob.glob(os.path.join(system.val_dir, '???.png'))
        ...
        imgs[0].save(os.path.join(system.val_dir, f'{hparams.exp_name}.gif'), save_all=True, loop=0, duration=50,
                        append_images=imgs[1:],)

        imgs_name = glob.glob(os.path.join(system.val_dir, '???_d.png'))
        imgs_name = sorted(imgs_name)
        imgs = list()
        for i in range(len(imgs_name)):
            imgs.append(Image.open(imgs_name[i]))
        ...
        imgs[0].save(os.path.join(system.val_dir, f'{hparams.exp_name}_d.gif'), save_all=True, loop=0, duration=50,
                        append_images=imgs[1:],)
